...
export const makeQBOClient = (
  config: z.infer<typeof zConfig>,
  settings: z.infer<typeof zSettings>,
  onSettingsChange: (settings: z.infer<typeof zSettings>) => void,
) => {
  const apiHost =
    config.url ??
  ...
  const creds$ = new rxjs.BehaviorSubject({
    ...
    refreshToken: settings.oauth.credentials.refresh_token,
  })
  function updateSettings() {
    console.log('[qbo] Updating settings')
    return onSettingsChange?.({
      ...
      oauth: {
        ...
        credentials: {
          ...
          access_token: creds$.value.accessToken,
          expires_at: creds$.value.accessTokenExpiresAt,
  ...
  const http = createHTTPClient({
    baseURL: `${apiHost}/v3/company/${creds$.value.realmId}/`,
    headers: {'Content-Type': 'application/json'},
    requestTransformer: (req) => {
      req.headers = {
        ...(req.headers as Record<string, unknown>),
        Authorization: `Bearer ${creds$.value.accessToken}`,
      }
      return req
    },
    ...
    refreshAuth: {
      shouldProactiveRefresh: () =>
        creds$.value.accessTokenExpiresAt
          ? // Proactive refresh within 30 mins.
            DateTime.fromISO(creds$.value.accessTokenExpiresAt) >=
      ...
      refresh: () =>
        oauth2()
          ...
          .then((res) => {
            creds$.next({realmId: creds$.value.realmId, ...parseTokens(res)})
  ...
  function globalId(id: string) {
    return `${creds$.value.realmId}_${id}`
  }
  ...
  return {
    ...
    getCompanyInfo: zFunction(async () =>
      http
        .get<QBO.GetCompanyInfoPayload>(`companyinfo/${creds$.value.realmId}`)
        .then((r) => r.data),
    ),
    ...
    revokeAccessToken: zFunction(async () => {
      const oauth = oauth2()
      const res = await oauth.revokeToken(creds$.value.accessToken)
      return res
    }),

    refreshAccessToken: zFunction(async () => {
      const oauth = oauth2()
      const res = await oauth.refreshToken(creds$.value.refreshToken!)
      creds$.value.accessToken = res.access_token
      return res
    }),
    SCOPES: {
