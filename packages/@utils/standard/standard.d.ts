/**
 * Replaces the previous `Interop`. ize format of representing data
 * that should can be used to import / data from all accounting programs
...
declare namespace Standard {
   ...
   */
  type AccountType = import('./standard-utils').AccountType
  type ReviewStatus = import('./standard-utils').ReviewStatus
  ...
  type AccountSubtype = _Split2<AccountType> | BrandedString<'AccountSubtype'>

  type Icon = {emoji: string; url?: never} | {emoji?: never; url: string}

  interface Account<TBalance extends Balance = Balance> {
    id?: Id.acct | null
    /** e.g. `Chase Checking` */
    name: string
    icon?: Icon | null
  ...
  interface Commodity<TPrice = Price> {
    id?: Id.comm | null
    /** symbol. `USD` */
    unit: Unit
    /** `US Dollar` */
  ...
  type PostingsMap<TPosting extends Posting | null = Posting> = Record<
    string,
    TPosting
  > &
    SpecialPostingsMap<TPosting>

  type Frequency = 'days' | 'weeks' | 'months' | 'quarters' | 'years'
  ...
  type PostingType = 'main' | 'fee' | 'trading' | 'category'
   ...
   * by the providers
   */
  interface Holding extends Amount {
  ...
  type LabelsMap = Record<string /* name */, boolean | string>
  ...
  type _Split1<S extends string> = S extends `${infer H}/${infer T}` ? H : never
  type _Split2<S extends string> = S extends `${infer H}/${infer T}` ? T : never
}
